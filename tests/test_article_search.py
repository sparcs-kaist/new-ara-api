...
@pytest.mark.usefixtures(
    "set_user_client", "set_index", "set_board", "set_authors", "set_posts"
)
    ...
    def test_main_search(self):
        ...
        def get_searched_article_number(q):
            return self.http_request(
                self.user, "get", "articles", querystring=f"main_search__contains={q}"
            ).data["num_items"]
        ...
        results = [
            (get_searched_article_number(query[0]), query[1]) for query in queries
        ]
