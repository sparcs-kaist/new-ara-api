...
class TestUserNickname(TestCase, RequestSetting):
    def test_nickname_update(self):
        ...
        assert self.user.nickname_updated_at is None
        ...
        assert (timezone.now() - self.user.nickname_updated_at).total_seconds() < 5
        ...
        # 3개월이 좀 안됐을 경우 오류 발생
        self.user.nickname_updated_at -= timedelta(months=2)
        self.user.save()
        r = self.http_request(self.user, 'put', f'user_profiles/{self.user.id}', data=update_data)
        assert r.status_code != 200

        # 3개월 이전일 경우 정상적으로 변경
        self.user.nickname_updated_at -= timedelta(months=1)
        self.user.save()
        r = self.http_request(self.user, 'put', f'user_profiles/{self.user.id}', data=update_data)
        assert r.status_code == 200
        self.user.refresh_from_db()
