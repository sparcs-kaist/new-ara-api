...
def set_block(request):
    request.cls.block = Block.objects.create(
        blocked_by=request.cls.user2, user=request.cls.user
    )
...
class TestScrap(TestCase, RequestSetting):
    ...
    def test_scrap_side_article_order(self):
        ...
        Scrap.objects.create(
            parent_article=self.article2, scrapped_by=self.user
        )  # 늦게 작성, 먼저 스크랩 된 글
        Scrap.objects.create(
            parent_article=self.article, scrapped_by=self.user
        )  # 먼저 작성, 늦게 스크랩 된 글
        ...
        res1 = self.http_request(
            self.user,
            "get",
            f"articles/{self.article2.id}",
            querystring="from_view=scrap",
        )
    ...
    def test_scrapped_by_multiple_user(self):
        ...
        Scrap.objects.create(
            parent_article=self.article2, scrapped_by=self.user2
        )  # 같은 글을 user2가 스크랩
    ...
    def test_scrap_cascade_delete_when_article_delete(self):
        ...
        self.http_request(
            self.user2, "delete", f"articles/{self.article2.id}"
        )  # 작성자가 article2 삭제
