...
class TestReport(TestCase, RequestSetting):
    ...
    def test_get_all_reports(self):
        ...
        assert (
            report1.get("parent_comment").get("id")
            == self.comment_report.parent_comment.id
        )
        assert (
            report1.get("reported_by").get("id") == self.comment_report.reported_by.id
        )
        ...
        assert (
            report2.get("parent_article").get("id")
            == self.article_report.parent_article.id
        )
        assert (
            report2.get("reported_by").get("id") == self.article_report.reported_by.id
        )
    ...
    def test_hide_article_if_many_reports(self):
        ...
        # 신고 2번 추가
        Report.objects.create(
            parent_article=self.article, reported_by=self.user2, content="test report 2"
        )

        Report.objects.create(
            parent_article=self.article, reported_by=self.user3, content="test report 3"
        )
        Report.objects.create(
            parent_article=self.article, reported_by=self.user4, content="test report 4"
        )
    ...
    def test_hide_comment_if_many_reports(self):
        ...
        # 신고 2번 추가
        Report.objects.create(
            parent_comment=self.comment, reported_by=self.user2, content="test report 2"
        )

        Report.objects.create(
            parent_comment=self.comment, reported_by=self.user3, content="test report 3"
        )

        Report.objects.create(
            parent_comment=self.comment, reported_by=self.user4, content="test report 4"
        )
